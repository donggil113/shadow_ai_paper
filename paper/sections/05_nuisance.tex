...
\begin{aligned}
  ...
  \le{}& \tfrac{\Gamma}{\varepsilon}\,\|\hat e-e\|_{2}\|\hat p_1-p_1\|_{2}\\
...
$[\hat\pi_{1/\Gamma}-z\hat s_{1/\Gamma},\ \hat\pi_{\Gamma}+z\hat s_{\Gamma}]$,
$z=z_{1-\alpha/2}$, covers the identified interval $[\pi_{1/\Gamma},\pi_\Gamma]$
with asymptotic probability at least $1-\alpha$.
...
\emph{inflated box}
\begin{equation}
  \I^{\circ}(x)=\big[\plo(e_L,\eta_L),\pup(e_L,\eta_U)\big](x)\supseteq[\plo(x),\pup(x)]
  \label{eq:inflated}
\end{equation}
...
both cover jointly with probability $\ge1-\alpha$ for every $k$, and the
coverage in (b) holds \emph{pointwise} on $B_k$ if $e,p_1$ are constant on
$B_k$, or after widening by the within-bin oscillation
$\omega_k=\sup_{x,x'\in B_k}|e(x)-e(x')|$ (resp.\ for $p_1$). No
assumption-free pointwise confidence interval for a regression function exists
\citep{low1997nonparametric}, so some such condition is unavoidable; (c) makes
it explicit.
\end{enumerate}
\end{theorem}

Part (a) is the double-machine-learning product rate
\citep{chernozhukov2018double} for the bound map: the aggregate endpoints are
estimable at $\sqrt n$ even when each nuisance converges slowly. Part (b)
says per-unit statements never enjoy this: the only way to certify a unit is
to certify its nuisances. Part (c) says how, and at what price. Proofs are in
\cref{app:nuisance}.
...
AUROC). ``bins'' is (c) with 20 bins (variants in
\cref{app:nuisance-variants}); ``AUROC'' is coverage of the deployment AUROC by the interval
...
reaches the nominal level with 20 bins where $p(x)$ is known, at roughly
twice the width, and lifts the AUROC interval's coverage of the realised
COMPAS AUROC to $\nExpEightCompasAucCovEmpBinsTwenty$. The aggregate route
of (a) needs no inflation: the one-step outer interval covers the identified
prevalence interval in every run while the plug-in never does. The diagnostic
rows locate the bottleneck: an oracle propensity barely moves coverage, an
oracle outcome model restores it --- as \eqref{eq:dr-bias} predicts, since
the propensity enters the bias only through the product term.
