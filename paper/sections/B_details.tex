...
\State initialise $\theta$ at the ridge fit to the midpoint $\tilde p$, normalised
\For{$t=1,\dots,T$}
  \If{$t \equiv 0 \pmod k$}
  ...
  \State $g \gets \overline{p_i(1-p_j)\,s(1-s)(x_i-x_j)}/\tau - \lambda\theta$
  \State $\theta \gets \Pi_{\|\theta\|=1}\big(\theta + \alpha\, g/\|g\|\big)$
\EndFor
\end{algorithmic}
\end{algorithm}

Because line 4 solves the inner minimisation \emph{exactly} rather than
approximately, Danskin's theorem makes the gradient in line 6 a valid
supergradient of $\min_{p\in\I_\Gamma}\AUC(\cdot,p)$ wherever the argmin is
unique, and the iterates converge to a stationary point of \eqref{eq:dcl}'s
ranking analogue.
