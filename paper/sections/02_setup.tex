...
$O=(X,\,T,\,T\cdot Y)$ --- never $S$, never $Y$ on $\{T=0\}$; $P$ is the
law of $(X,S,T,Y)$ and $P_{\mathrm{obs}}$ that of $O$.

\paragraph{Identification.} $P_{\mathrm{obs}}$ identifies
$e(x)=\Prb(T=1\mid X=x)$ and $p_1(x)=\Prb(Y=1\mid X=x,\,T=1)$ but not the
deployment target $p(x)=\Prb(Y=1\mid X=x) = e\,p_1 + (1-e)\,p_0$, because
$p_0(x)=\Prb(Y=1\mid X=x,\,T=0)$ never enters $P_{\mathrm{obs}}$. We target the \emph{deployment risk}
$R_P(f)=\E_P[\ell(f(X),Y)]$ of a score $f:\X\to\R$ over all units, not the
\emph{observed risk} $\E_P[\ell(f(X),Y)\mid T=1]$ practitioners measure.
...
Read: \emph{given everything the model sees, the decision maker's
private information shifts the censored units' outcome log-odds by at most
$\log\Gamma$.} $\Gamma=1$ is missing at random; $\Gamma=\infty$ recovers
the Manski bounds. $\dcsm$ is the decision-censoring
analogue of Rosenbaum's sensitivity analysis \citep{rosenbaum1983assessing,
rosenbaum2002observational} and matches Tan's marginal sensitivity model
\citep{tan2006distributional} on the propensity:
...
\Cref{lem:msm} (proof in \cref{app:setup}) lets us work in $\Gamma$
($\Gamma=\Lambda^2$ on the propensity scale).
...
First, the set is a \emph{product} of intervals, so separable optimisation
decouples across $x$ (\cref{sec:learning}). Second, $\Gamma$ enters \eqref{eq:tilt} as a
\emph{log-odds translation}, so error in $p_1$ reaches the bounds unamplified
--- hence $\log\Gamma$, not $\Gamma$, in \cref{sec:generalization}. Let
$\tilde p = (\plo+\pup)/2$ and $\delta = \pup - \plo$ be the midpoint and the
\emph{identification width}, a per-unit measure of what censoring hides.
